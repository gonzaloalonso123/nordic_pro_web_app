...
export const useFormPrograms = <TData = FormProgramRow[]>(
  options?: Omit<UseQueryOptions<FormProgramRow[], Error, TData>, "queryKey" | "queryFn">
) => {
...
export const useFormProgram = <TData = FormProgramRow>(
  ...
  options?: Omit<UseQueryOptions<FormProgramRow | null, Error, TData>, "queryKey" | "queryFn" | "enabled">
) => {
  return useQuery<FormProgramRow | null, Error, TData>({
    queryKey: ["form-programs", programId],
    queryFn: () => (programId ? formProgramsService.getById(supabase, programId) : null),
    enabled: !!programId,
    ...options,
  });
};
...
export const useFormProgramsByCategory = <TData = FormProgramRow[]>(
  ...
  options?: Omit<UseQueryOptions<FormProgramRow[], Error, TData>, "queryKey" | "queryFn" | "enabled">
) => {
  return useQuery<FormProgramRow[], Error, TData>({
    queryKey: ["form-programs", "category", categoryId],
    queryFn: () => (categoryId ? formProgramsService.getByCategory(supabase, categoryId) : []),
    enabled: !!categoryId,
    ...options,
  });
};
...
export const useCreateFormProgram = (
  options?: Omit<UseMutationOptions<FormProgramRow, Error, FormProgramInsert>, "mutationFn">
) => {
...
export const useUpdateFormProgram = (
  options?: Omit<
    UseMutationOptions<FormProgramRow, Error, { programId: string; updates: FormProgramUpdate }>,
    "mutationFn"
  >
) => {
  const queryClient = useQueryClient();

  return useMutation<FormProgramRow, Error, { programId: string; updates: FormProgramUpdate }>({
    mutationFn: ({ programId, updates }) => formProgramsService.update(supabase, programId, updates),
    onSuccess: (data, variables, context) => {
      queryClient.invalidateQueries({ queryKey: ["form-programs"] });
...
export const useDeleteFormProgram = (options?: Omit<UseMutationOptions<boolean, Error, string>, "mutationFn">) => {
  const queryClient = useQueryClient();
