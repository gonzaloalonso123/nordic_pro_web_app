...
export const useFormsByEvent = <TData = FormRow[]>(
  ...
  return useQuery<FormRow[] | null, Error, TData>({
    ...
    queryFn: () => (eventId ? formsService.getByEvent(supabase, eventId) : null),
    enabled: !!eventId,
    ...options,
  })
}
...
export const useCreateForm = (options?: Omit<UseMutationOptions<FormRow, Error, FormInsert>, "mutationFn">) => {
  ...
  return useMutation<FormRow, Error, FormInsert>({
    ...
    onSuccess: (data, variables, context) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["forms", "user", data.user_id] })
      queryClient.invalidateQueries({ queryKey: ["forms", "event", data.event_id] })
...
export const useSoftDeleteForm = (options?: Omit<UseMutationOptions<boolean, Error, string>, "mutationFn">) => {
  const supabase = useSupabaseBrowser()
  const queryClient = useQueryClient()
...
export const useHardDeleteForm = (options?: Omit<UseMutationOptions<boolean, Error, string>, "mutationFn">) => {
  const supabase = useSupabaseBrowser()
  const queryClient = useQueryClient()
